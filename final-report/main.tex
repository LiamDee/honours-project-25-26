...
Code snippet \ref{lst:loadUsage} shows the function used to fetch the usage data for the user. This function is called on app startup, as it gets called within the onCreate method. The function calls the
...
Code snippet \ref{lst:isOnboardingCompleteCheck} shows the code used for checking if the user has completed the onboarding process, and is called every time the user initialises the app. The code uses a
...
in code snippet \ref{lst:edittext} below where the structure for the goal entry screen is defined. XML files in android app development define the layout for a UI, and house contain components such as
...
A complete collection of the responses and their associated questions can be found in Appendix O, however, the sentiment derived from 
