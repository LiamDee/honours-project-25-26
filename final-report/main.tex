...
% maybe remove urls from references - makes it look cleaner
% sort appendices properly
...
of diagram was decided upon due to how easily it can convey the idea of how a requirement should function (cite). Flowchart diagrams were the exclusive diagram used for designing the application.
...
some requirements were made before others as some requirements couldn't be implemented before others were implemented first

\subsubsection{Proof-of-Concept Prototype(s)}

\begin{lstlisting}[language=Java, style=mystyle, caption={some code}]
...
is being tested. %cite 
As the nature of development involved creating a component based on the requirements specified, and only continuing once the component had been created to match the specification, this
method of testing felt most appropriate.
...
\noindent When evaluating if the PRI met the project's hypothesis, objective, and research statement, a mixed-method approach was used. (define mixed-method approach)
The evaluation results are based off of the captured usage data from each participant, as well as the participant's own opinions of their experience using the 
PRI via a survey given at the end of the evaluation period. While the obtained usage stats will be the main focus, the results from the survey help
to provide additional context and insights that quantitative data cannot easily do.
...
there was a shift in usage of social media apps. \newline %tying back to the fact that the phone could be used for productivity use, not just social media (quote og source)
...
The y-axis shows the usage in hours. Table \ref{tab:3c-usage-sample} shows a sample of the usage data captured, showing the name of the app package (unique identifier for android apps), the usage time
...
Within the dataset, an outlier was detected within the baseline data for participant 3C. Seen above in table \ref{tab:3c-usage-sample}, the the "android" 
package, (referring to the home screen? quote a source) takes up over 24 hours of usage time. Seeing as this may be unrealistic for a typical user, the decision was
...
https://github.com/LiamDee/honours-project-25-26/tree/main/documentation/requirements-specification
...
the GitHub repository (see Appendix A).\newline

\textbf{Iteration Req-1.1}
