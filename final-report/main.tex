...
Figure 1 showcases the process behind implementing and testing the PRI. When building prototypes of the PRI, a GitHub branch that matched the name of the requirement was created, then, within
that branch, that requirement was implemented, then tested upon via Incremental Integration Testing to ensure the requirement was implemented as intended. If that part of the requirement passed all test
cases as intended, a new GitHub branch for the next requirement was created using the newly completed requirement as a blueprint, and this process repeated until the implementation of 
...
An example of how testing was carried out can be seen in Table 2. The test case includes the test ID, mapped to match the requirement number, the 
...
Figure 2 shows the initial results from the evaluation, with the total usage from each participant. The x-axis shows the timeline of the evaluation period, from baseline to week 3. 
The y-axis shows the usage in hours. Table 3 shows a sample of the usage data captured, showing the name of the app package (unique identifier for android apps), the usage time
...
Within the dataset, an outlier was detected within the baseline data for participant 3C. Seen above in table 3, the the "android" 
...
made to leave out the usage time for "android". Figure 3 below shows the same chart as Figure 2, but without the outlier data.
...
Figure 4 shows the same graph as Figure 3, but only including social media usage from each participant. While the graph is relatively the same, there are some
differences. For example, unlike the total usage, 1A's social media usage never increases, rather, taking a small decline instead. 4D's social media usage starts, and
remains quite low, though does take a small increase in week-1 before levelling out by the latter weeks.
...
implemented the WDEP system to a good degree. As demonstrated in Figure 3, only two of the respondents felt that the app contributed to their
...
\subsection*{Appendix D - Software Requirements Specification}
